...
class AudioSegment(object):
    ...
    def __init__(self, data=None, *args, **kwargs):
        ...
        if self.sample_width == 3:
            ...
            # Workaround for python 2 vs python 3.
            # _data in 2.x are length-1 strings,
            # and in 3.x are ints.
    ...
    def from_mono_audiosegments(cls, *mono_segments):
        ...
        if segs[0].channels != 1:
            raise ValueError(
                ...
                " to be mono AudioSegment instances")
    ...
    def from_file(cls, file, format=None, codec=None,
                  parameters=None, use_temp_files=False, **kwargs):
        orig_file = file
        ...
        def is_format(f):
            ...
            if retval:
                if isinstance(retval, bytes):
                    enc_func = lambda x: x.encode('utf8')
                else:
                    enc_func = lambda x: x
                retval = retval.lower().endswith(enc_func(".{0}".format(f)))
        ...
        if use_temp_files:
            ...
            if info:
                ...
                # This is a workaround for some ffprobe versions that always
                # say that mp3/mp4/aac/webm/ogg files contain fltp samples
        ...
        conversion_command += [
            ...
            output.name if use_temp_files else "-"
        ]
        ...
        fmt_string = "Decoding failed. ffmpeg returned error code: " + \
        ...
        if use_temp_files:
            ...
            try:
                if p.returncode != 0:
                    raise CouldntDecodeError(fmt_string.format(p.returncode,
                                                               p_err))
